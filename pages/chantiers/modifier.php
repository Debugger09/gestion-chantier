...
// Récupération des clients et chefs
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $db->prepare("UPDATE chantiers SET nom_chantier = ?, localisation = ?, date_debut = ?, date_fin = ?, client_id = ?, id_chef = ? WHERE id = ?");
        $stmt->execute([$nom_chantier, $localisation, $date_debut, $date_fin, $client_id, $id_chef, $id]);
        
        $_SESSION['message'] = "Chantier modifié avec succès";
...
<div class="ajouter-container">
    ...
    <a href="liste.php" class="lien-retour">Retour à la liste</a>
        <div class="form-group">
            ...
            <input type="text" name="nom_chantier" value="<?= htmlspecialchars($_POST['nom_chantier'] ?? $chantier['nom_chantier']) ?>" required>
        </div>
        
        <div class="form-group">
            <label>Localisation</label>
            <input type="text" name="localisation" value="<?= htmlspecialchars($_POST['localisation'] ?? $chantier['localisation']) ?>" required>
        </div>
        
        <div class="form-row">
            <div class="form-group">
                <label>Date de début</label>
                <input type="date" name="date_debut" value="<?= htmlspecialchars($_POST['date_debut'] ?? $chantier['date_debut']) ?>" required>
            </div>
            
            <div class="form-group">
                <label>Date de fin (optionnel)</label>
                <input type="date" name="date_fin" value="<?= htmlspecialchars($_POST['date_fin'] ?? $chantier['date_fin']) ?>">
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="client_id" required>
                ...
                <?php foreach ($clients as $client): ?>
                    <option value="<?= $client['id'] ?>" <?= ($_POST['client_id'] ?? $chantier['client_id']) == $client['id'] ? 'selected' : '' ?>>
        ...
        <div class="form-group">
            ...
            <select name="id_chef">
                ...
                <?php foreach ($chefs as $chef): ?>
                    <option value="<?= $chef['id'] ?>" <?= ($_POST['id_chef'] ?? $chantier['id_chef']) == $chef['id'] ? 'selected' : '' ?>>
